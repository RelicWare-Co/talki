 ...
 */
export const luciaDbMiddleware: Get<[], UniversalMiddleware> =
 ...
 * @link {@see https://lucia-auth.com/guides/validate-session-cookies/}
 */
export const luciaAuthContextMiddleware: Get<[], UniversalMiddleware> =
	() => async (request, context) => {
		const sessionId = context.lucia.readSessionCookie(
	...
	return (response: Response) => {
		if (context.session?.fresh) {
			response.headers.append(
				"Set-Cookie",
				context.lucia.createSessionCookie(context.session.id).serialize(),
			);
		}
		if (context.sessionId && !context.session) {
			response.headers.append(
				"Set-Cookie",
				context.lucia.createBlankSessionCookie().serialize(),
			);
		}
 ...
 */
export const luciaAuthSignupHandler = (() =>
	async (request, context, _runtime) => {
		...
		try {
			await drizzleQueries.signupWithCredentials(
				context.db,
				userId,
				username,
				passwordHash,
			...
			return new Response(
				JSON.stringify({ error: { invalid: "An unknown error has occurred" } }),
				{
					status: 500,
					headers: {
						"content-type": "application/json",
 ...
 */
export const luciaAuthLoginHandler = (() =>
	async (request, context, _runtime) => {
		const body = (await request.json()) as {
			username: string;
			password: string;
		...
		const existingUser: DatabaseUser | undefined | null =
		...
		const validPassword =
			...
			(await scrypt.verify(existingUser.password, password));
...
export const luciaAuthLogoutHandler = (() => async (_request, context) => {
	...
	[],
	UniversalMiddleware<Universal.Context & { session?: Session | null }>
>;
